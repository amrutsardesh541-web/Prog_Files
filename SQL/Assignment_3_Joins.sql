...
-- list students details along with department name in alphabetical order of their name inner join using
select * from student
inner join department
using (dept_id)
order by s_name asc;

-- list student details who belong to Pune city and whose department asset is more than 50 lakh inner join on
select * from student 
inner join department
on student.dept_id = department.dept_id
where student.city = "Pune" and
department.asset > 5000000;


-- display employee name in capital letter whose name consist of *ji also print them in reverse order
SELECT UPPER(e_name) AS name 
FROM employee 
WHERE e_name LIKE "%ma%"
ORDER BY e_name DESC;


-- list all students name, dob, if they are placed their packages and year of placement using left
select * from placement_record
left join student
using (stu_id); 

-- display student_id package placed along with company name country display company details using right join
select * from placement_record
right join company
using(comp_id);

-- find student_name and with highest package 
SELECT * FROM student 
INNER JOIN placement_record  
ON student.stu_id = placement_record.stu_id
WHERE placement_record.package = (SELECT MAX(package) FROM placement_record);

-- list hod details along with department name whose intake is more than 100 using natural join
select * from employee
natural join department
where department.intake >= 100 and employee.hod_id is not null;

-- list company details who have recruited students in year 2024 
select s.stu_id, c.comp_id, c.comp_name, pr.year_of_start
from student s
join placement_record pr using(stu_id)
join company c on pr.comp_id = c.comp_id
where pr.year_of_start = 2024;

-- list students from MECH department who are placed with a package of more than 6 lakh
select * from student as s
inner join placement_record as pr
using(stu_id)
where s.dept_id = "MECH" and pr.package > 600000;

-- list student_name who are placed in Infosys 
select * from student as s
join placement_record as pr
using(stu_id)
join company as c on 
c.comp_id = pr.comp_id
where c.comp_name = "Infosys";

SELECT pr.stu_id, pr.comp_id, pr.package, c.comp_name
FROM placement_record AS pr
INNER JOIN company AS c USING (comp_id)
WHERE pr.package > (
    SELECT max(pr2.package)
    FROM placement_record AS pr2
    INNER JOIN company AS c2 ON pr2.comp_id = c2.comp_id
    WHERE c2.comp_name = 'Tata Consultancy Services'
);
