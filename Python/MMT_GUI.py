...
def get_recommendations(start_city, destination, package_type, person, want_airline, cosine_sim=cosine_sim):
    ...
    canvas = tk.Canvas(output_window, bg='#87CEEB')
...
#from_city_option = ttk.Combobox(frame, values=["Mumbai"])
#from_city_label = tk.Label(frame, text="Start City", bg="white", fg="black")
#from_city_label.grid(row=0, column=, padx=10, pady=5)
from_city_entry = tk.Entry(frame)
from_city_entry.grid(row=1, column=0, padx=10, pady=5)
...
#calling the output
def call():
    start_city=from_city_entry.get()
